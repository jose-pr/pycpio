...
class CPIOHeaderKwargs(LoggedKwargs, total=False):
    name: str
    ino: int
    mtime: float
...
class CPIOHeader(Logged):
    ...
    def __init__(
        self, header_data=b"", overrides={}, *args, **kwargs: Unpack[CPIOHeaderKwargs]
    ):
    ...
    def from_args(self, *args, **kwargs: Unpack[CPIOHeaderKwargs]) -> None:
    ...
    def __setattr__(self, key: str, value):
        ...
        if key == "filesize" and value != b"00000000":
            if getattr(self, "filesize", b"00000000") not in [value, b"00000000"]:
                self.logger.warning(
                    "[%s] Changed filesize: %s -> %s"
                    % (self.name, self.filesize, value)
                )
        ...
        if key == "name":
            ...
            if hasattr(self, "namesize") and namesize != int(self.namesize, 16):
                self.logger.debug(
                    "Name size changed: %s -> %s" % (int(self.namesize, 16), namesize)
                )
            self.namesize = namesize

    def process_overrides(self, overrides: CPIOHeaderKwargs) -> None:
        ...
        for attribute in self.structure:
            if attribute in overrides:
                self.logger.log(
                    5, "[%s] Pre-override: %s" % (attribute, getattr(self, attribute))
                )
                if attribute == "mode":
    ...
    def __str__(self):
        ...
        for attr in self.structure:
            if attr in [
                ...
                if self.logger.level < 20:
                    out_str += (
                        f"    {attr}: {datetime.fromtimestamp(int(self.mtime, 16))}\n"
                    )
                else:
                    continue
            else:
